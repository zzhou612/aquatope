...
def main(args):
    ...
    dst_name = Path(video_name).stem + '_watermarked' + Path(video_name).suffix
    dst_path = Path('/tmp') / dst_name
    cmd = [ffmpeg_path, '-y', '-i', video_url,
           '-preset', 'superfast', str(dst_path)]
    cmd = [ffmpeg_path, '-y', '-i', video_url, '-i',
           str(png_path), '-filter_complex', 'overlay=10:10', str(dst_path)]
    ...
    # Return result
    # --------------------------------------------------------------------------
    return {'watermarked_video': result.object_name}
